...
struct timeval g_latest_ts;
size_t pkt_counter = 0;
struct timeval g_start_ts;
...
void nids_pcap_handler(u_char * par, struct pcap_pkthdr *hdr, u_char * data)
{
	...
	//gettimeofday(&g_latest_ts, NULL);
	g_latest_ts = hdr->ts;

	if(pkt_counter++ == 0) {
		g_start_ts = g_latest_ts;		
	}
...
struct tcp_timeout *nids_udp_timeouts = 0;
...
start:
    ...
    tcp_init(nids_params.n_tcp_streams);
   	udp_init(nids_params.n_udp_streams);
