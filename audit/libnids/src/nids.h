...
extern struct timeval g_latest_ts;
extern struct timeval g_start_ts;
...
struct udp_stream {
	...
	struct udp_stream *next_node;
	struct udp_stream *pre_node;
	int hash_index;
  	struct udp_stream *next_time;
  	struct udp_stream *prev_time;
	struct udp_stream *next_free;
	char nids_state; /* for udp, there are only three states, NIDS_DATA/NIDS_TIMED_OUT/NIDS_EXITING */
    char bypass;
};
