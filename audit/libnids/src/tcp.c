...
static struct tcp_stream *add_new_tcp(struct tcphdr *this_tcphdr, struct ip *this_iphdr)
{
    ...
    a_tcp->client.traffic_count = 0;
    a_tcp->server.traffic_count = ntohs(this_iphdr->ip_len);

    memcpy(a_tcp->dst_mac, (char *)this_iphdr - nids_linkoffset, 6);
    memcpy(a_tcp->src_mac, (char *)this_iphdr - nids_linkoffset + 6, 6);
...
static void add_from_skb(struct tcp_stream * a_tcp, struct half_stream * rcv,
    struct half_stream * snd, u_char *data, int datalen, u_int this_seq, char fin, char urg, u_int urg_ptr)
{
...
static void tcp_queue(struct tcp_stream * a_tcp, struct tcphdr * this_tcphdr,
    struct half_stream * snd, struct half_stream * rcv, char *data, int datalen, int skblen)
{
...
struct tcp_stream *find_stream(struct tcphdr *this_tcphdr, struct ip *this_iphdr, int *from_client)
{
    ...
    reversed.saddr =ntohl(this_iphdr->ip_dst.s_addr);
    reversed.daddr =ntohl(this_iphdr->ip_src.s_addr);
...
void process_tcp(u_char * data, int skblen)
{
    ...
    if (my_tcp_check(this_tcphdr, iplen - 4 * this_iphdr->ip_hl, this_iphdr->ip_src.s_addr, \
    ...
    snd->traffic_count += htons(this_iphdr->ip_len);
...
#if 0
// |¨¬?¨¤¡§?D?o??¡§o?¨¤?¨º??¡§¡ã???¡è?¨¦?¡§a??|¨¬?seq2?|¨¬¡§¡§¡§?¡§2rcv|¨¬?ack_seq¡§o?¨¤?¨º?¡§¡ãa¡§¡§¡ê¡è?¡§?2¡§|?¡è?¨¦?¡§a??|¨¬?seq¡§o?2?¡§o?2????¨º??a¡§¡è??¡§|¡§¡ã?1y??¡§¡ã?D??????|¨¬???¡§1??¨º
